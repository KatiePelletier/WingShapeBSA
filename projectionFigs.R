...
gmaxDs <- ggplot(x1dat, aes(x, y)) + 
  ...
  annotate("text", x = 0.28, y = 0.6, label =  TeX(r"($ \textit{r} = \frac{\textbf{ds} \cdot \textbf{g_{max}}}{||\textbf{ds}|| \times ||\textbf{g_{max}}||}$)"), size = 8) + 
  xlab(expression(x[1])) +
  ylab(expression(x[2])) +
  theme_classic() +
  xlim(0, 1) +
...
scatterplot <- ggplot(points, aes(x = points[,1], y = points[,2])) + 
  ...
  xlab(expression(paste("PC1 represents ", italic(g))[paste(italic(max))]))  + 
        theme(axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.y = element_blank(),
        axis.text.y = element_blank(),
